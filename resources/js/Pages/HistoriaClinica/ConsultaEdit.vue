<template>
  <AppLayout>
    <div class="space-y-6">
      ...
      <p class="mb-4 text-sm text-gray-600 dark:text-gray-400">Paciente: <strong>{{ paciente.nombres }} {{ paciente.apellidos }}</strong>. Edite los datos de la consulta.</p>
      <form @submit.prevent="form.post('/actualizar-consulta')" class="space-y-4">
        <input type="hidden" name="paciente_id" :value="paciente.id" />
        <input type="hidden" name="consulta_id" :value="consulta.id" />
        <div><label class="mb-1.5 block text-sm font-semibold text-gray-800 dark:text-gray-200">Fecha de consulta <span class="text-red-500">*</span></label><input v-model="form.fecha_consulta" type="datetime-local" class="input-hc" required /></div>
        ...
        <div class="flex flex-wrap gap-3">
          <button type="submit" class="rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600">Guardar cambios</button>
